...
def get_args():
    ...
    parser.add_argument("--log_path", type=str, default="tensorboard/double_train_pr_A0.6_B0.4")
    parser.add_argument("--saved_path", type=str, default="trained_models/double_train_pr_A0.6_B0.4")
    parser.add_argument("--target_update_freq", type=int, default=200, help="The number of steps between target network's updates")
    parser.add_argument("--alpha", type=float, default=0.6, help="alpha in replay")
    parser.add_argument("--beta", type=float, default=0.4, help="beta in replay")
...
def train(opt):
    ...
    model_target.eval()
    replay_memory = ReplayBuffer(opt.replay_memory_size, opt.alpha, opt.beta, opt.num_iters)
    iter = 0
    while iter < opt.num_iters:
        ...
        batch, indices, weight = replay_memory.sample(opt.batch_size)
        ...
        y_batch = torch.cat(
            tuple(reward if terminal else reward + opt.gamma * prediction[max_action] for reward, terminal, prediction, max_action in
                  zip(reward_batch, terminal_batch, next_prediction_batch, torch.argmax(model(next_state_batch), axis=1))))

        q_value = torch.sum(current_prediction_batch * action_batch, dim=1)

        replay_memory.update(indices, y_batch.detach().cpu().numpy()-q_value.detach().cpu().numpy())
        optimizer.zero_grad()
        # y_batch = y_batch.detach()
        if opt.beta == 0:
            loss = criterion(q_value, y_batch)
        else:
            loss = ((y_batch-q_value).pow(2)*weight).mean()
            replay_memory.update_beta(iter)
        loss.backward()
        optimizer.step()
        ...
        iter += 1
        if iter % 100 == 0:
            print("PR raw: Iteration: {}/{}, Action: {}, Loss: {}, Epsilon {}, Reward: {}, Q-value: {}".format(
                iter + 1,
                opt.num_iters,
                action,
                loss,
